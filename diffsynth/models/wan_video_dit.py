...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        add_control_adapter: bool = False, # False: put the control_adapter into vace instad of dit
    ...
    def patchify(self, x, control_camera_latents_input, control_adapter):
        x = self.patch_embedding(x)
        if control_adapter is not None and control_camera_latents_input is not None:
            # for cfg concat
            control_camera_latents_input = torch.cat([control_camera_latents_input, control_camera_latents_input], dim=2)
