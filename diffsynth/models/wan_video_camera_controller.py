...
def process_pose_file_direct(ext=None, intr=None, width=672, height=384, original_pose_width=1280, original_pose_height=720, device='cpu', return_poses=False):
        intrinsic = np.asarray([[intr[0,0],
                                intr[1,1],
                                intr[2,0],
                                intr[2,1]
                                ]], dtype=np.float32)
        ...
        K = torch.as_tensor(intrinsic)[None]  # [1, 1, 4]  
        c2ws = torch.as_tensor(ext)[None]  # [1, n_frame, 4, 4]
        plucker_embedding = ray_condition(K, c2ws, height, width, device=device)[0].permute(0, 3, 1, 2).contiguous()  # V, 6, H, W
        plucker_embedding = plucker_embedding[None]
        plucker_embedding = rearrange(plucker_embedding, "b f c h w -> b f h w c")[0]
        return plucker_embedding
...
def generate_camera_coordinates(
            ...
            if direction.dim() == 1:
                ...
                control_bs = direction.shape[0]
                coordinates_all = []

            for b_size in range(control_bs):
                decoded = [MOTION_LABELS[i.item()] for i in direction] if direction.dim() == 1 else \
                        [MOTION_LABELS[i.item()] for i in direction[b_size]]

                coordinates = [list(map(float, origin))]

                for i_th, step in enumerate(decoded):
                    coor = coordinates[-1].copy()
                    if step == "Left":
                        coor[9] += speed[i_th].item()
                    elif step == "Right":
                        coor[9] -= speed[i_th].item()
                    elif step == "Up":
                        coor[13] += speed[i_th].item()
                    elif step == "Down":
                        coor[13] -= speed[i_th].item()
                    elif step == "In":
                        coor[18] -= speed[i_th].item()
                    elif step == "Out":
                        coor[18] += speed[i_th].item()
                    elif step == "LeftUp":
                        coor[9] += speed[i_th].item()
            ...
            return coordinates if direction.dim() == 1 else coordinates_all