...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def switch_pipe_to_training_mode(
        ...
        if lora_base_model is not None:
            ...
            if trainable_models == 'vace':
                trainable_submodules = ["control_adapter"]
                for name, param in model.named_parameters():
                    if any(t in name for t in trainable_submodules):
...
class ModelLogger:
    ...
    def on_step_end(self, accelerator, model, save_steps=None):
        ...
        if save_steps is not None and self.num_steps % save_steps == 0:
            ...
            # self.save_model(accelerator, model, f"step-{self.num_steps}_cfg.safetensors")
            # self.save_model(accelerator, model, f"step-{self.num_steps}_concat_new_cfg.safetensors")
            self.save_model(accelerator, model, f"step-{self.num_steps}_concat_new.safetensors")
            # self.save_model(accelerator, model, f"step-{self.num_steps}_ref_img.safetensors")
    ...
    def on_training_end(self, accelerator, model, save_steps=None):
        if save_steps is not None and self.num_steps % save_steps == 0:
            ...
            # self.save_model(accelerator, model, f"step-{self.num_steps}_cfg_concat.safetensors")
